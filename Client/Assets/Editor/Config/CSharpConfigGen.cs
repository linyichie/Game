...
public static class CSharpConfigGen {
    ...
    public static void Generate(FileInfo fileInfo) {
        ...
        if (lines.Length > 2) {
            ...
            GenerateConfig(new ConfigClassInfo() {
                className = StringUtil.Concat(fileInfo.Name.Substring(0, fileInfo.Name.IndexOf('.')), "Config"),
    ...
    static string GetFieldLine(string field, string type) {
        ...
        switch (type) {
            default:
                return StringUtil.Concat("public ", type, " ", field, " { get; private set; }");
        }
    }
    ...
    static string GetParseLine(string field, string type, int index) {
        ...
        switch (type) {
            case "string":
                return StringUtil.Concat(field, string.Format(" = values[{0}]", index), ".Trim();");
            case "int":
                return StringUtil.Concat(field, string.Format(" = ConfigParse.ParseInt(values[{0}].Trim())", index),
                    ";");
        }
    ...
    static void GenerateConfig(ConfigClassInfo classInfo) {
        var path = StringUtil.Concat("Assets/Scripts/Game/Config/", classInfo.className, ".cs");
